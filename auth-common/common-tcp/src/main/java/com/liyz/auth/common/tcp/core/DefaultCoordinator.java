...
public class DefaultCoordinator extends AbstractTCInboundHandler implements TransactionMessageHandler, Disposable {
     ...
     */
    protected static final long COMMITTING_RETRY_PERIOD = 1000L;

    /**
     * The constant ASYNC_COMMITTING_RETRY_PERIOD.
     */
    protected static final long ASYNC_COMMITTING_RETRY_PERIOD = 1000L;

    /**
     * The constant ROLLBACKING_RETRY_PERIOD.
     */
    protected static final long ROLLBACKING_RETRY_PERIOD = 1000L;

    /**
     * The constant TIMEOUT_RETRY_PERIOD.
     */
    protected static final long TIMEOUT_RETRY_PERIOD = 1000L;

    /**
     * The Transaction undo log delete period.
     */
    protected static final long UNDO_LOG_DELETE_PERIOD = 24 * 60 * 60 * 1000;
    ...
    protected void doGlobalBegin(GlobalBeginRequest request, GlobalBeginResponse response, RpcContext rpcContext)
        ...
        if (log.isInfoEnabled()) {
            log.info("Begin new global transaction applicationId: {},transactionServiceGroup: {}, transactionName: {},timeout:{},xid:{}",
    ...
    protected void timeoutCheck() throws TransactionException {
        ...
        if (allSessions.size() > 0 && log.isDebugEnabled()) {
            log.debug("Global transaction timeout check begin, size: {}", allSessions.size());
        }
        for (GlobalSession globalSession : allSessions) {
            ...
            log.info("Global transaction[{}] is timeout and will be rollback.", globalSession.getXid());
        ...
        if (allSessions.size() > 0 && log.isDebugEnabled()) {
            log.debug("Global transaction timeout check end. ");
        }
    ...
    protected void handleRetryRollbacking() {
        ...
        for (GlobalSession rollbackingSession : rollbackingSessions) {
            try {
                ...
                log.info("Failed to retry rollbacking [{}] {} {}", rollbackingSession.getXid(), ex.getCode(), ex.getMessage());
            }
        }
    }
    ...
    protected void handleRetryCommitting() {
        ...
        for (GlobalSession committingSession : committingSessions) {
            try {
                ...
                log.info("Failed to retry committing [{}] {} {}", committingSession.getXid(), ex.getCode(), ex.getMessage());
            }
        }
    }
    ...
    protected void handleAsyncCommitting() {
        ...
        for (GlobalSession asyncCommittingSession : asyncCommittingSessions) {
            try {
                ...
                log.error("Failed to async committing [{}] {} {}", asyncCommittingSession.getXid(), ex.getCode(), ex.getMessage(), ex);
            }
        }
    }
    ...
    protected void undoLogDelete() {
        ...
        if (rmChannels == null || rmChannels.isEmpty()) {
            if (log.isDebugEnabled()) {
                log.debug("no active rm channels to delete undo log");
            }
            return;
        }
        short saveDays = UndoLogDeleteRequest.DEFAULT_SAVE_DAYS;
        for (Map.Entry<String, Channel> channelEntry : rmChannels.entrySet()) {
            ...
            try {
                ...
                log.error("Failed to async delete undo log resourceId = {}, exception: {}", resourceId, e.getMessage());
            }
        }
    }
    ...
    public void init() {
        ...
        undoLogDelete.scheduleAtFixedRate(() -> {
            try {
                ...
                log.info("Exception undoLog deleting ... ", e);
            }
