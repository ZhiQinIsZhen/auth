...
import com.liyz.auth.common.tcp.v1.session.SessionManager;
...
public class SessionHolder {
    ...
    public static void init(String mode) {
        ...
        if (StoreMode.DB.equals(storeMode)) {
            ...
            String sessionStorePath = DEFAULT_SESSION_STORE_FILE_DIR;
            if (StringUtils.isBlank(sessionStorePath)) {
                throw new RemoteServiceException("the {store.file.dir} is empty.");
            }
                        ...
                        if (storeMode == StoreMode.FILE) {
                            ...
                            switch (globalStatus) {
                                ...
                                case Begin:
                                    ...
                                    break;
                                default:
                                    throw new RemoteServiceException("NOT properly handled " + globalStatus);
                            }
                        }
                        break;
                    }
    ...
    private static void removeInErrorState(GlobalSession globalSession) {
        try {
            log.warn("The global session should NOT be {}, remove it. xid = {}", globalSession.getStatus(), globalSession.getXid());
            ROOT_SESSION_MANAGER.removeGlobalSession(globalSession);
            if (log.isInfoEnabled()) {
                log.info("Remove global session succeed, xid = {}, status = {}", globalSession.getXid(), globalSession.getStatus());
            }
        } catch (Exception e) {
            log.error("Remove global session failed, xid = {}, status = {}", globalSession.getXid(), globalSession.getStatus(), e);
        }
    }
    ...
    public static SessionManager getRootSessionManager() {
        if (ROOT_SESSION_MANAGER == null) {
            throw new RemoteServiceException("SessionManager is NOT init!");
        }
